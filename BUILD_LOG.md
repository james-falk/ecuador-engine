...
13. [ ] **DEFERRED — blocked on #14 + James OAuth grant** — B2B account-creation automation (Frieda's / Melissa's / wholesale portal signup). Playwright scripts that fill supplier/vendor application forms on 2-3 distributor portals. Output: per-portal "draft application" JSON written to a new `b2b_account_applications` table (status=`draft`, may auto-submit ONLY if James pre-approves the portal in `b2b_account_applications.allow_submit` flag). Reads sign-in / verification codes from inbox via #14 helper and continues form-fill autonomously. Smoke test = fixture-driven form-fill end-to-end verified with mock email-code arrival. **Untag DEFERRED once: (a) #14 is built and (b) James has re-granted Google OAuth with Gmail scope.**
14. [ ] **Gmail integration — read inbox + extract sign-in codes** — extend the existing Google OAuth client (already wired for Drive on jamesfalk4@gmail.com — see `scripts/google-oauth-setup.md`) with `https://www.googleapis.com/auth/gmail.readonly` scope. Build `src/lib/gmail/list-messages.ts` (recent inbox fetch, From/Subject/Date metadata + body snippet) + `src/lib/gmail/extract-code.ts` (regex helpers for common code shapes: 6-digit numeric, alphanumeric 6-8, "your code is X", magic-link URL extraction) + API route `src/app/api/gmail/await-code/route.ts` that polls inbox up to 5 min for a sign-in code matching a per-call selector (sender domain + subject pattern + age window). Smoke test = fixture Gmail API response → extract code → assert correct value. **JAMES ACTION REQUIRED to unblock #13:** when this item ships, the existing OAuth flow at `/api/google/login` (or whatever local-dev route it uses) needs re-running so the consent screen presents the new Gmail scope and James grants it. Without that re-grant, the API route returns 403 even after deploy.
...
### 2026-05-03 — Item #5: Buyer-scout core + dedupe
Built the buyer-scout agent skeleton: `src/lib/agents/buyer-scout/dedupe.ts` (canonicalization helpers + 3-tier SHA256 `computeDedupeKey` per BUILD_LOG dedupe contract — website > email-domain > name-only), `src/lib/agents/buyer-scout/sources/customs-manifest-leads.ts` (mines 30d shipment rows out of `pricing_snapshots` where `source='importyeti'`, aggregates by consignee, scores 50 base + 20 if competitor origin + 10 if 2+ BoLs), `src/lib/agents/buyer-scout/run.ts` (orchestrator with cross-table dedupe — queries both `lead_proposals.dedupe_key` AND `companies` WHERE kind='buyer' with on-the-fly hashing since companies is don't-touch), and API route `src/app/api/agents/buyer-scout/route.ts`. Smoke test `scripts/smoke-buyer-scout.ts` covers canonicalization (apostrophe stripping, suffix stripping like inc/llc/produce), tier fallback, identical-identity hash collision, different-company hash divergence, source consignee aggregation (4 shipments → 2 candidates with correct scores), and a fixture-driven agent dry-run. All 5 check groups pass; `pnpm typecheck` clean.

CODEX OUTAGE: codex `--yolo` token refresh failed again (`refresh_token_reused` / 401). Fell back to direct implementation by openclaw-coding-agent. Five nights running; codex re-auth still pending.
...
### 2026-05-02 — Gmail integration item added (manual edit, post-cron)
James clarified he wants the autonomous agent to do **the entire B2B account-creation flow**, including grabbing sign-in / verification codes from his inbox. Added Item #14: Gmail integration — extend the existing Google OAuth scopes with `gmail.readonly`, build a list/extract-code helper, expose as `/api/gmail/await-code` polling route. Updated Item #13 to DEFERRED until #14 ships AND James re-grants OAuth with the new scope. So the cron now picks #5 → #6 → #7 → #8 → #10 → #14, then #13 unblocks once James re-grants.
